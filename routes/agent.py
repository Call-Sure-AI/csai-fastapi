...
# Create router without prefix since it's added in routes/__init__.py
...
@router.get("/agent/", response_model=List[Agent])
...
async def test_agent_router():
    """Test endpoint to verify agent router is working"""
...
@router.get("/agent/user/{user_id}", response_model=List[Agent])
...
@router.post("/agent/", response_model=Agent, status_code=201)
